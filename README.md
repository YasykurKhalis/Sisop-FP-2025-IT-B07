...
generated with the seq-rd-32th-32f workload. We
use seq-rd-32th-32f as a reference for the figure
because this workload has more requests per operation
type compared to other workloads. Bars are ordered
from left to right by the appearance of requests in the experiment. The same request types, but in different quantities, were generated by the other read-intensive workloads [rows #1–20]. For the single threaded read workloads, only one request per LOOKUP, OPEN, FLUSH, and
...
count remained the same across all workloads; and finally GETATTR is invoked before unmount for the root
directory and was the same for all the workloads.
...
4. Tarasov, V., Vangoor, B. R., & Zadok, E. (2017). *To FUSE or Not to FUSE: Performance of User-Space File Systems*. In Proceedings of the 15th USENIX Conference on File and Storage Technologies (FAST '17), pp. 59–72. USENIX Association. https://www.usenix.org/system/files/conference/fast17/fast17-vangoor.pdf
